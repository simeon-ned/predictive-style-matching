"""Wrapper for mjlab's play script that auto-registers psm tasks.
...
For ``Psm-G1``, the frozen predictor next to that checkpoint
(``<run>/params/predictor``) is selected automatically inside
``PsmVelocityCommand`` by reading ``sys.argv``—no play-script patching.
...
Usage:
  ...
  python scripts/play.py Psm-G1 --checkpoint-file /path/to/model.pt
  python scripts/play.py Psm-G1 --agent zero
"""
