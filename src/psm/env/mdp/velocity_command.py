...
class UniformVelocityCommand(CommandTerm):
  ...
  def _snap_small_commands_to_zero(
    self, env_ids: torch.Tensor | None = None
  ) -> None:
    """Zero small planar / yaw commands after sampling or heading updates.

    Keeps rewards, duty-cycle gating, and observations consistent without
    duplicating deadbands in each reward. Set a deadband to ``0.0`` to disable.
    """
    lin_db = self.cfg.lin_vel_deadband
    ang_db = self.cfg.ang_vel_deadband
    if lin_db <= 0.0 and ang_db <= 0.0:
      return
    if env_ids is not None and env_ids.numel() == 0:
      return
    mask: slice | torch.Tensor = slice(None) if env_ids is None else env_ids
    v = self.vel_command_b
    if lin_db > 0.0:
      xy = v[mask, :2]
      small_xy = torch.norm(xy, dim=1, keepdim=True) < lin_db
      v[mask, :2] = torch.where(small_xy, torch.zeros_like(xy), xy)
    if ang_db > 0.0:
      wz = v[mask, 2]
      small_w = torch.abs(wz) < ang_db
      v[mask, 2] = torch.where(small_w, torch.zeros_like(wz), wz)
  ...
  def _resample_command(self, env_ids: torch.Tensor) -> None:
    ...
    self._snap_small_commands_to_zero(env_ids)
  ...
  def _update_command(self) -> None:
    ...
    self._snap_small_commands_to_zero()
  ...
  def _debug_vis_impl(self, visualizer: "DebugVisualizer") -> None:
    ...
    for batch in env_indices:
      ...
      if np.linalg.norm(base_pos_w) < 1e-6:
        continue
      def local_to_world(vec: np.ndarray, pos: np.ndarray = base_pos_w, mat: np.ndarray = base_mat_w) -> np.ndarray:
        return pos + mat @ vec
      ...
      cmd_ang_to = local_to_world((np.array([0, 0, z_offset]) + np.array([0, 0, cmd[2]])) * scale)
      visualizer.add_arrow(cmd_lin_from, cmd_ang_to, color=(0.2, 0.6, 0.2, 0.6), width=0.015)
      act_lin_from = local_to_world(np.array([0, 0, z_offset]) * scale)
      act_lin_to = local_to_world((np.array([0, 0, z_offset]) + np.array([lin_vel_b[0], lin_vel_b[1], 0])) * scale)
      visualizer.add_arrow(act_lin_from, act_lin_to, color=(0.0, 0.6, 1.0, 0.7), width=0.015)
      act_ang_to = local_to_world((np.array([0, 0, z_offset]) + np.array([0, 0, ang_vel_b[2]])) * scale)
      visualizer.add_arrow(act_lin_from, act_ang_to, color=(0.0, 1.0, 0.4, 0.7), width=0.015)
